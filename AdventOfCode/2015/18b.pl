...
my $sum = 0;
$sum += sum @{$_} for(@grid);
# Since the last operation of the loop is assignment of grid to next, the corners might be turned off.
$sum += 4 - ($grid[1][1] + $grid[1][$len - 2] + $grid[$len - 2][1] + $grid[$len - 2][$len - 2]);
say $sum;
